...
class SpotlessRoombaState(RoombaState):
    ...
    def readFromFile(filename: str) -> SpotlessRoombaState:
        ...
        with open(filename, 'r') as file:
            ...
            grid = tuple(tuple(CLEAN_TERRAIN.get(Terrain(x), Terrain(x))
    ...
    def __init__(self,
        ...
        """
        super().__init__(position=position, grid=grid, parent=parent,
                         last_action=last_action, depth=depth, path_cost=path_cost)
    ...
    def get_next_state(self, action: RoombaAction) -> SpotlessRoombaState:
        ...
        return SpotlessRoombaState(
            dirty_locations=tuple(
                x for x in self.dirty_locations if x != new_pos)
